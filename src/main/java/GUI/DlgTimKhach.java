...
public class DlgTimKhach extends JDialog {
    ...
    private void xuLyChonKhachHang() {
        ...
        if (row > -1) {
            ...
            double tongChiTieu = Double.parseDouble(tblKhachHang.getValueAt(row, 4) + "");

            customer = customerService.getCustomerById(ma);
        }
        this.dispose();
    }
    ...
    private void loadDataLenTable() {
        ...
        List<Customer> lstCustomer = customerService.getAllCustomers();
        if (lstCustomer != null) {
            for (Customer item : lstCustomer) {
                Vector vec = new Vector();
                vec.add(item.getCustomerId());
                vec.add(item.getFullName());
                vec.add(item.getPhone());
                vec.add(item.getGender());
                vec.add(item.getTotalSpent());
    ...
    private void loadDataLenTable(String tuKhoa) {
        dtmKhachHang.setRowCount(0);
        List<Customer> lstCustomer = customerService.getCustomerByName(tuKhoa);
        for (Customer item : lstCustomer) {
                Vector vec = new Vector();
                vec.add(item.getCustomerId());
                vec.add(item.getFullName());
