...
import BLL.InvoiceDetailService;
...
public class ExportBillForm extends JDialog {
    ...
    public ExportBillForm(ArrayList<Vector> dsGioHang, int tongTien, Object nhanVien) {
        this();
    ...
    private void xuLyHienThiHoaDon() {
        ...
        hd += "</tr>";
        if (timMaUI.promotion != null) {
            double percent = 0;
            // lấy phần trăm giảm
            percent = timMaUI.promotion.getDiscountPercent();
            if (tongTien >= timMaUI.promotion.getMinOrderAmount()) {
                tongTien = tongTien - (tongTien * percent / 100);
            } else {
                JOptionPane.showMessageDialog(null, "Không đủ điều kiện nhận ưu đãi!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
        ...
        hd += "<td style='text-align:center;'>" + timMaUI.promotion.getDiscountPercent()+ "%" + "</td>";
        hd += "</tr>";
        hd += "<tr>";
        hd += "<td style='text-align:center;'>" + "</td>";
        hd += "<td style='text-align:left;'>" + "</td>";
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (txtTenKhach.getText().equals("")) {
            JOptionPane.showMessageDialog(null, "Vui lòng chọn khách hàng", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
...
//        }
        ...
        invoice.setEmployeeId(Session.employeeId);
        invoice.setCustomerId(DlgTimKhach.customer.getCustomerId());
        invoice.setTotalAmount(tongTien);
        invoice.setPromotionId(DlgTimMaGiam.promotion.getPromoId());
        
        int invoiceId = invoiceService.createInvoice(invoice);
        if(invoiceId == -1)
        {
            JOptionPane.showMessageDialog(null, "Thêm hóa đơn thất bại", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        for (Vector vec : dsGioHang) {
            int maSP = Integer.parseInt(vec.get(0) + "");
            int soLuong = Integer.parseInt(vec.get(2) + "");
            double donGia = Double.parseDouble(vec.get(3) + "") ;
            double thanhTien = Double.parseDouble(vec.get(4) + "");
            var detail = new InvoiceDetail();
            detail.setInvoiceId(invoiceId);
            detail.setProductId(maSP);
            detail.setQuantity(soLuong);
            detail.setUnitPrice(donGia);
    ...
    private void btnTimKhachActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (timKhachUI.customer != null) {
            txtTenKhach.setText(timKhachUI.customer.getCustomerId()+ " - " + timKhachUI.customer.getFullName());
        }
    }
    ...
    private void btnTimMaGiamActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (timMaUI.promotion != null) {
            txtMaGiam.setText(timMaUI.promotion.getPromoId() + " - " + timMaUI.promotion.getName());
        }
    }
